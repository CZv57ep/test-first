"""
    ...
    Defining the merging firm with the larger share as buyer under a GUPPI safeharbor bound of 6%,
    and the merging firm with the *smaller* share as the buyer under a GUPPI safeharbor bound of 5%,
    and incrementing shares and margins by 5%, plot mergers that clear the safeharbor threshold, color-coded
    by margin of the firm with the larger GUPPI estimate.
...
sample_sz = 10**6
qtyshr_disttype = "Uniform"
pcm_disttype = "Uniform"

base_path = data_path.joinpath(
    "{}.zip".format("_".join((prog_path.stem, f"gbar{g_bar * 100:02.0f}PCT")))
)

save_data_to_file_flag = False

h5path = data_path.joinpath(f"{prog_path.stem}.h5")
blosc_filters = tb.Filters(complevel=3, complib="blosc:lz4", fletcher32=True)
h5datafile = tb.open_file(
    h5path, mode="a", title="Datasets, Sound GUPPI Safeharbor", filters=blosc_filters
)
...
def gen_plot_data(
    ...
    *,
    ...
    if _guppi_aggr_method not in (_aggr_ok := ("Maximum", "Average")):
        raise ValueError(
            f"Invalid argument, {_guppi_aggr_method!r}; must be one of {_aggr_ok!r}."
        )

    h5hier = f"/generated_plotdata/pcmsame{_pcm_same_flag}"

    if _pcm_same_flag:
        _pcm_array = _pcm_firm2_star * np.ones_like(qtyshr_array)
    else:
        _pcm_firm2 = _pcm_firm2_star * np.ones_like(qtyshr_array[:, [1]])
        _pcm_array = np.column_stack((
            np.divide(
                np.einsum("ij,ij->ij", _pcm_firm2, 1 - qtyshr_array[:, [1]]),
                1 - qtyshr_array[:, [0]],
            ),
            _pcm_firm2,
        ))
    ...
    if _guppi_aggr_method.lower().startswith("max"):
        _guppi_est = _guppi_array.max(axis=1, keepdims=True)
    else:
        _guppi_est = _guppi_array.mean(axis=1, keepdims=True)
    ...
    del _qtyshr_firm1_safe, _qtyshr_firm2_safe, _pcm_sorter
...
def main():  # noqa PLR0915
    ...
    print(
        ...
        "by Firm 2 margin," "coded by Firm 1 margin",
        sep=" ",
    )
    ...
    my_fig_2dsg = plt.figure(figsize=(8.5, 9.5))

    fig_grid = my_fig_2dsg.add_gridspec(
        nrows=1,
        ncols=2,
        figure=my_fig_2dsg,
        width_ratios=[6, 0.125],  # height_ratios=[1.0,],
        wspace=0.0,
    )
    fig_grid_gsf = fig_grid[0, 0].subgridspec(nrows=3, ncols=1, wspace=0, hspace=0.125)

    my_fig_3dsc = plt.figure(figsize=(5, 5))
    ...
    for ax_col, ax_collbl in enumerate(("Maximum", "Average", "Minimum")[:1]):
        for ax_row, ax_rowtup in enumerate((
            ...
            ax_now.text(
                ...
                "\n".join([
                    r"$m_2 = m^* = {0:.{1}f}\%$".format(
                        (_pcmv := pcm_firm2_star * 100), int(1 * (_pcmv % 1 > 0))
                    ),
                    r"$m_1 \neq m_2$",
                ]),
                rotation=0,
            ...
            if ax_collbl == "Maximum":
                ...
                my_fig_3dsc.savefig(my_fig_3dsc_savepath, dpi=1200)
    ...
    my_fig_2dsg_savepath = base_path.parent.joinpath(
        f"{base_path.stem}_2DScatterGrid.pdf"
    )
    print(f'Save 2D plot to, "{my_fig_2dsg_savepath}"')
    my_fig_2dsg.savefig(my_fig_2dsg_savepath, dpi=1200)
